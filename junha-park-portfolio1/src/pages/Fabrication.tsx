...
const builds = [
  ...
  },
  {
    ...
    description: 'A seamless fusion of additive manufacturing and embedded systems. Features a custom PCB and a 3D-printed enclosure optimized for thermal dissipation and light diffusion.',
    tags: ['3D Printing', 'PCB Design', 'Microcontroller'],
    images: ['/Photos/ToolBox.jpg', '/Photos/Tools.jpg'],
    aspect: '21/9'
  }
];
      ...
      <div className="space-y-40">
        {builds.map((build) => (
          <section
            ...
            className={
              build.reverse
                ? 'bg-surface-container-low/50 -mx-8 px-8 py-24 border-y border-outline-variant/10'
                : ''
            }
          >
            <div
              className={`grid grid-cols-12 gap-8 lg:gap-16 items-center ${
                build.reverse ? 'lg:flex-row-reverse' : ''
              }`}
            >
              <div
                className={`col-span-12 lg:col-span-8 ${
                  build.reverse ? 'lg:order-2' : ''
                }`}
              >
                ...
                {build.tags && (
                  <div className="flex flex-wrap gap-2 pt-4">
                    {build.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 bg-surface-container-highest text-on-surface-variant text-[9px] uppercase font-bold tracking-[0.15em] rounded-full border border-outline-variant/20"
                      >
                        {tag}
                      </span>
                    ))}
